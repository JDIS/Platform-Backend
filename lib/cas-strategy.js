...
CASStrategy.prototype.authenticate = function (req, options) {
  ...
  if (req.query && req.query[this._options.cas.parameters.ticket]) {
    const url = self._options.baseUrl + self._options.cas.urls.validate +
    ...
    const url = self._options.baseUrl + self._options.cas.urls.login +
      '?' + self._options.cas.parameters.service + '=' +
      encodeURIComponent(protocol + '://' + req.host + self._options.callbackUrl);
...
// This is because req.protocol function from koa doesn't have the app context in this request.
